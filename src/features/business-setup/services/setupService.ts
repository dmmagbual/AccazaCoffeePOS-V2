...
export const businessSetupSchema = z.object({ idempotencyKey: z.string().regex(/^[A-Za-z0-9_-]{8,150}$/), step: z.number().int().min(1).max(14), status: z.enum(['not_started', 'in_progress', 'initialized', 'cancelled', 'failed']), profile: profileSchema, organizationName: z.string().trim().min(2), branch: branchSchema, tax: z.object({ name: z.string().trim().min(2), rate: z.string().regex(/^0(?:\.\d{1,6})?$|^1(?:\.0{1,6})?$/), mode: z.enum(['inclusive', 'exclusive', 'exempt']) }), chartTemplate: z.enum(['coffee_shop', 'import_existing']), enabledFeatures: z.array(z.string()), owner: z.object({ name: z.string().trim().min(2), email: z.string().trim().email(), password: z.string().min(12) }), updatedAt: z.string().datetime() })
