...
export type SaleFailureCode = 'empty_cart' | 'invalid_quantity' | 'pricing_mismatch' | 'payment_mismatch' | 'payment_reference_missing' | 'operational_error' | 'persistence_error' | 'duplicate_submission' | 'offline_unavailable'
export interface SaleError { code: SaleFailureCode; message: string; recoverable: boolean }
export interface SaleInput { organizationId: string; storeId: string; cashierId: string; shiftId?: string; items: readonly CartItem[]; discount: CartDiscount | null; taxRate: number; customerId?: string; payments: readonly Payment[]; notes?: string; saleTimestamp: Date }
...
export interface CompletedSaleOrder { id: string; orderNumber: string; status: 'completed'; organizationId: string; storeId: string; cashierId: string; shiftId?: string; customerId?: string; items: readonly CompletedSaleItem[]; subtotal: number; discount: number; tax: number; total: number; estimatedCogs: number; notes?: string; saleTimestamp: Date }
