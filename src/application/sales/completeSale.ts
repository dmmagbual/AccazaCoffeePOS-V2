...
export interface CompleteSaleDependencies { validateOrder: (input: { items: readonly { product: SaleInput['items'][number]['product']; quantity: number; modifierIds?: readonly string[] }[]; saleDate: Date }) => OperationalValidationResult; buildSnapshot: (product: SaleInput['items'][number]['product'], modifierIds: readonly string[], saleDate: Date) => CompletedSaleItem['snapshot']; persistence: SalePersistence; business: Receipt['business'] }
...
function idempotencyKey(input: SaleInput): string {
  const supplied = input.idempotencyKey?.trim()
  if (supplied && /^[A-Za-z0-9_-]{8,150}$/.test(supplied)) return supplied
  return `sale_${input.storeId}_${input.cashierId}_${input.payments.map((payment) => payment.id).sort().join('_')}`.replace(/[^A-Za-z0-9_-]/g, '_').slice(0, 150)
}
export async function completeSale(input: SaleInput, dependencies: CompleteSaleDependencies): Promise<CompleteSaleResult> {
  const submissionKey = idempotencyKey(input)
  ...
  try {
    ...
    const readiness = dependencies.validateOrder({ items: input.items.map((item) => ({ product: item.product, quantity: item.quantity, modifierIds: item.modifierIds })), saleDate: input.saleTimestamp })
    ...
    const change = round(paid - summary.grandTotal); const orderId = submissionKey; const orderNumber = identifier('ORD', input.storeId, input.saleTimestamp)
    const items: CompletedSaleItem[] = input.items.map((item) => { const snapshot = dependencies.buildSnapshot(item.product, item.modifierIds ?? [], input.saleTimestamp); const proportion = summary.subtotal === 0 ? 0 : item.itemTotal / summary.subtotal; return { productId: item.product.id, productName: item.product.name, sku: item.product.sku, barcode: item.product.barcode, sellingPrice: item.product.pricing.sellingPrice, quantity: item.quantity, discountAllocation: round(summary.discount * proportion), taxAllocation: round(summary.tax * proportion), snapshot, estimatedItemCogs: round(snapshot.totalEstimatedCogs * item.quantity), lineTotal: item.itemTotal, saleTimestamp: input.saleTimestamp } })
    const estimatedCogs = round(items.reduce((sum, item) => sum + item.estimatedItemCogs, 0)); const order: CompletedSaleOrder = { id: orderId, idempotencyKey: submissionKey, orderNumber, status: 'completed', organizationId: input.organizationId, storeId: input.storeId, cashierId: input.cashierId, shiftId: input.shiftId, customerId: input.customerId, items, payments: input.payments, subtotal: summary.subtotal, discount: summary.discount, tax: summary.tax, total: summary.grandTotal, estimatedCogs, notes: input.notes, saleTimestamp: input.saleTimestamp }
