...
const item: CartItem = { product: { id: 'product-a', name: 'Client-only name', description: '', category: { id: 'coffee', name: 'Coffee', description: '', active: true }, sku: 'SKU-A', barcode: 'BAR-A', favorite: false, available: true, modifiers: [], variants: [], inventory: { trackInventory: true, stockQuantity: 1, minimumStock: 0, maximumStock: 10, unit: 'piece' }, pricing: { cost: 1, sellingPrice: 99, currency: 'PHP' }, tax: { taxable: true, taxRate: 0.12 }, image: { url: '', alt: '' }, size: '', unit: 'piece', tags: [], status: 'active' }, variationId: 'variation-a', selectedOptionItemIds: [{ optionItemId: 'option-shot', quantity: 1 }], quantity: 2, note: 'no foam', itemTotal: 198 }

describe('trusted POS sale client', () => {
  it('sends only identifier-based checkout intent to the callable', () => {
    const request = toTrustedSaleRequest({ branchId: 'branch-a', shiftId: 'shift-a', items: [item], payments: [{ id: 'payment-a', method: 'Cash display label', paymentMethodId: 'configured-cash-id', amount: 200, currencyCode: 'PHP', tenderedAmount: 200 }], idempotencyKey: 'checkout-key-a' })
    expect(request).toEqual({ idempotencyKey: 'checkout-key-a', requestedBranchId: 'branch-a', shiftId: 'shift-a', cartLines: [{ clientLineId: 'product-a-1', productId: 'product-a', variationId: 'variation-a', quantity: 2, selectedOptionItemIds: [{ optionItemId: 'option-shot', quantity: 1 }], notes: 'no foam' }], payments: [{ paymentMethodId: 'configured-cash-id', amount: 200, currencyCode: 'PHP', tenderedAmount: 200 }] })
    ...
    expect(JSON.stringify(request)).not.toContain('Cash display label')
  })
