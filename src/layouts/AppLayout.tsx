    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 lg:flex">
      <aside className="flex w-full flex-col bg-slate-950 p-4 text-slate-300 lg:fixed lg:inset-y-0 lg:w-64">
        ...
        <nav aria-label="Primary navigation">
          ...
          <NavLink to={ROUTES.settings} className={({ isActive }) => `mt-1 flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition ${isActive ? 'bg-slate-800 text-white' : 'hover:bg-slate-900 hover:text-white'}`}><SlidersHorizontal size={18} />Settings</NavLink>
          ...
          <NavLink to={ROUTES.recipeStudio} className={({ isActive }) => `mt-1 flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition ${isActive ? 'bg-slate-800 text-white' : 'hover:bg-slate-900 hover:text-white'}`}><BookOpenCheck size={18} />Recipe studio</NavLink>
