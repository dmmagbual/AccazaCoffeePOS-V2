...
exports.OptionItemRepository = void 0;
...
class OptionItemRepository {
    ...
    async read(id) { const snapshot = await this.db.collection('optionItems').doc(id).get(); if (!snapshot.exists)
        throw new https_1.HttpsError('not-found', 'Option item was not found.'); return { id: snapshot.id, ...snapshot.data() }; }
    ...
    async listActiveByGroup(organizationId, branchId, groupId) { const query = await this.db.collection('optionItems').where('organizationId', '==', organizationId).where('modifierGroupId', '==', groupId).where('active', '==', true).limit(100).get(); return query.docs.map((doc) => ({ id: doc.id, ...doc.data() })).filter((item) => (0, validation_js_1.isBranchAvailable)(item.branchAvailability, branchId)); }
    async validateOptionForGroup(organizationId, branchId, groupId, optionId, quantity = 1) { const option = await this.getById(organizationId, optionId); if (!option.active || option.modifierGroupId !== groupId || !(0, validation_js_1.isBranchAvailable)(option.branchAvailability, branchId) || quantity <= 0 || (option.maximumQuantity !== null && option.maximumQuantity !== undefined && quantity > option.maximumQuantity))
        throw new https_1.HttpsError('failed-precondition', 'Option item is invalid for this selection.'); (0, validation_js_1.assertPositiveEffects)(option.ingredientEffects); return option; }
    async validateBranchAvailability(organizationId, branchId, optionId) { const option = await this.getById(organizationId, optionId); if (!(0, validation_js_1.isBranchAvailable)(option.branchAvailability, branchId))
        throw new https_1.HttpsError('failed-precondition', 'Option item is unavailable for this branch.'); return option; }
...
exports.OptionItemRepository = OptionItemRepository;
