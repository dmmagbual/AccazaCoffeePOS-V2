...
export class AvailabilityRepository {
    ...
    async resolve(organizationId, branchId, target, at, timezone) {
        ...
        const entries = query.docs.map((doc) => ({ id: doc.id, ...doc.data() })).filter((entry) => isEffective(entry.effectiveFrom, entry.effectiveTo, at) && ((target.productId && entry.productId === target.productId) || (target.variationId && entry.variationId === target.variationId) || (target.optionItemId && entry.optionItemId === target.optionItemId)));
