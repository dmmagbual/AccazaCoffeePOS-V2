...
exports.OptionGroupRepository = void 0;
...
class OptionGroupRepository {
    ...
    async read(id) { const snapshot = await this.db.collection('optionGroups').doc(id).get(); if (!snapshot.exists)
        throw new https_1.HttpsError('not-found', 'Option group was not found.'); return { id: snapshot.id, ...snapshot.data() }; }
    ...
    assignments(organizationId, field, id, branchId) { return this.db.collection('productOptionAssignments').where('organizationId', '==', organizationId).where(field, '==', id).where('active', '==', true).limit(50).get().then((query) => query.docs.map((doc) => ({ id: doc.id, ...doc.data() })).filter((assignment) => (0, validation_js_1.isBranchAvailable)(assignment.branchAvailability, branchId))); }
    ...
    async validateRequiredSelections(organizationId, groupId, selected, assignment) { const rules = await this.resolveSelectionRules(organizationId, groupId, assignment); if (rules.required && selected.length === 0)
        throw new https_1.HttpsError('failed-precondition', 'A required option group has no selection.'); }
    async validateMinimumSelections(organizationId, groupId, selected, assignment) { const rules = await this.resolveSelectionRules(organizationId, groupId, assignment); if (selected.length < rules.minimumSelections)
        throw new https_1.HttpsError('failed-precondition', 'Minimum option selections were not met.'); }
    async validateMaximumSelections(organizationId, groupId, selected, assignment) { const rules = await this.resolveSelectionRules(organizationId, groupId, assignment); if (selected.length > rules.maximumSelections || (rules.selectionType === 'SINGLE' && selected.length > 1))
        throw new https_1.HttpsError('failed-precondition', 'Maximum option selections were exceeded.'); }
    async resolveOptionGroupSnapshot(organizationId, branchId, id) { const group = await this.getById(organizationId, id); if (!group.active || !(0, validation_js_1.isBranchAvailable)(group.branchAvailability, branchId))
        throw new https_1.HttpsError('failed-precondition', 'Option group is unavailable.'); return { optionGroupId: group.id, optionGroupCode: group.code, optionGroupName: group.name }; }
}
exports.OptionGroupRepository = OptionGroupRepository;
