...
export class VariationRepository {
    ...
    async validateVariationForProduct(organizationId, branchId, productId, variationId) { const variation = await this.getById(organizationId, variationId); if (variation.productId !== productId || !variation.active || (variation.status !== undefined && variation.status !== null && variation.status !== 'ACTIVE' && variation.status !== 'PUBLISHED') || !isBranchAvailable(variation.branchAvailability, branchId) || !isEffective(variation.effectiveFrom, variation.effectiveTo, new Date()))
