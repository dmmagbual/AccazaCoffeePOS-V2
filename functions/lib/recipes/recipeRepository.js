...
class RecipeRepository {
    ...
    async recipe(id) { const doc = await this.db.collection('recipes').doc(id).get(); if (!doc.exists)
        throw new https_1.HttpsError('not-found', 'Recipe was not found.'); return { id: doc.id, ...doc.data() }; }
    async getRecipeById(organizationId, id) { const value = await this.recipe(id); if (value.organizationId !== organizationId)
        throw new https_1.HttpsError('permission-denied', 'Recipe is outside organization scope.'); return value; }
    async getVersionById(organizationId, recipeId, versionId) { const doc = await this.db.collection('recipes').doc(recipeId).collection('versions').doc(versionId).get(); if (!doc.exists)
        throw new https_1.HttpsError('not-found', 'Recipe version was not found.'); const value = { id: doc.id, ...doc.data() }; if (value.organizationId !== organizationId || value.recipeId !== recipeId)
        throw new https_1.HttpsError('permission-denied', 'Recipe version is outside organization scope.'); return value; }
    validateEffectiveDate(version, at = new Date()) { if (!(0, validation_js_1.isEffective)(version.effectiveFrom, version.effectiveTo, at))
        throw new https_1.HttpsError('failed-precondition', 'Recipe version is not effective.'); }
    validatePublishedStatus(recipe, version) { if (!recipe.published || recipe.recipeStatus !== 'PUBLISHED' || version.status !== 'published')
        throw new https_1.HttpsError('failed-precondition', 'Recipe is not published.'); }
    validateProtectedHeadOfficeRecipe(recipe, organizationId) { if (recipe.protectedHeadOfficeRecipe && recipe.sourceOrganizationId && recipe.sourceOrganizationId !== organizationId)
        throw new https_1.HttpsError('permission-denied', 'Protected Head Office recipe cannot be overridden.'); }
    async resolve(organizationId, productId, variationId) { const query = await this.db.collection('recipes').where('organizationId', '==', organizationId).where('productId', '==', productId).where('published', '==', true).limit(25).get(); const candidates = query.docs.map((doc) => ({ id: doc.id, ...doc.data() })).filter((recipe) => (variationId ? recipe.productVariationId === variationId : !recipe.productVariationId)); if (candidates.length !== 1)
        throw new https_1.HttpsError('failed-precondition', 'Exactly one published recipe must apply.'); const recipe = candidates[0]; const version = await this.getVersionById(organizationId, recipe.id, recipe.activeVersionId ?? ''); this.validatePublishedStatus(recipe, version); this.validateEffectiveDate(version); this.validateProtectedHeadOfficeRecipe(recipe, organizationId); return { recipe, version }; }
