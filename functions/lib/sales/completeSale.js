...
export const completeSaleRequestHash = (command) => JSON.stringify({ branchId: command.requestedBranchId, shiftId: command.shiftId, cartLines: command.cartLines, payments: command.payments, customerId: command.customerId ?? null, requestedRedemptionPoints: command.requestedRedemptionPoints ?? 0, notes: command.notes ?? null });
...
const calculateTax = (amount, tax) => {
    const precision = tax.roundingPrecision;
    const factor = 10 ** precision;
    const grossMinor = BigInt(Math.round(amount * factor));
    const noTax = tax.taxType === 'ZERO_RATED' || tax.taxType === 'EXEMPT' || tax.taxType === 'NO_TAX' || tax.calculationMode === 'NOT_APPLICABLE';
    const [rate, rateScale] = decimalRatio(tax.rateApplied);
    const taxMinor = noTax ? 0n : tax.calculationMode === 'TAX_INCLUSIVE' ? roundDivision(grossMinor * rate, rateScale + rate) : roundDivision(grossMinor * rate, rateScale);
    const exclusiveMinor = tax.calculationMode === 'TAX_INCLUSIVE' ? grossMinor - taxMinor : grossMinor;
    const inclusiveMinor = tax.calculationMode === 'TAX_INCLUSIVE' ? grossMinor : grossMinor + taxMinor;
    const toAmount = (value) => Number(value) / factor;
...
const assertRequest = (input) => {
    ...
    if (input.payments.some((payment) => !payment.paymentMethodId || payment.amount <= 0 || !payment.currencyCode) || new Set(input.payments.map((payment) => payment.paymentMethodId)).size !== input.payments.length)
        throw new HttpsError('invalid-argument', 'Payment input is invalid.');
    if (input.requestedRedemptionPoints !== undefined && (!Number.isSafeInteger(input.requestedRedemptionPoints) || input.requestedRedemptionPoints <= 0))
        throw new HttpsError('invalid-argument', 'Loyalty redemption points are invalid.');
};
export class FirestoreTrustedSaleRepository {
    ...
    async execute(command, context, resolved) {
        ...
        return this.db.runTransaction(async (transaction) => {
            ...
            const grossGrandTotal = round(lineTotals.reduce((sum, line) => sum + line.net, 0));
            const taxTotal = round(lineTotals.reduce((sum, line) => sum + line.tax, 0));
            const redemptionResolution = command.requestedRedemptionPoints ? await loyalty.resolve({ organizationId: context.organizationId, branchId: command.requestedBranchId, customerId: command.customerId, netSales: grossGrandTotal, requestedRedemptionPoints: command.requestedRedemptionPoints, now: context.requestTimestamp }) : null;
            const redemptionAmount = redemptionResolution?.redemptionAmount ?? 0;
            const grandTotal = round(grossGrandTotal - redemptionAmount);
            const paymentRepository = new PaymentMethodRepository(this.db);
            const validatedTenders = await Promise.all(command.payments.map((payment) => paymentRepository.validateTender(context.organizationId, command.requestedBranchId, payment, context.requestTimestamp)));
            const cashTenders = validatedTenders.filter((payment) => payment.isCash);
            if (cashTenders.length > 1)
                throw new HttpsError('failed-precondition', 'Only one cash tender is permitted.');
            const nonCashTotal = round(validatedTenders.filter((payment) => !payment.isCash).reduce((sum, payment) => sum + payment.amount, 0));
            ...
            const requiredCash = round(grandTotal - nonCashTotal);
            if (!cashTenders.length && requiredCash !== 0)
                throw new HttpsError('failed-precondition', 'Payments do not reconcile to the authoritative amount due.');
            const cashTender = cashTenders[0];
            const tenderedCash = cashTender ? (cashTender.tenderedAmount ?? cashTender.amount) : 0;
            if (cashTender && tenderedCash < requiredCash)
                throw new HttpsError('failed-precondition', 'Cash tender is insufficient.');
            const changeAmount = round(cashTender ? tenderedCash - requiredCash : 0);
            const persistedPayments = validatedTenders.map((payment) => ({ ...payment, amount: payment.isCash ? requiredCash : payment.amount, changeAmount: payment.isCash ? changeAmount : 0 }));
            if (persistedPayments.some((payment) => payment.amount <= 0))
                throw new HttpsError('failed-precondition', 'Payments do not reconcile to the authoritative amount due.');
            ...
            if (command.customerId) {
                try {
                    const loyaltyResolution = redemptionResolution ?? await loyalty.resolve({ organizationId: context.organizationId, branchId: command.requestedBranchId, customerId: command.customerId, netSales: grandTotal, now: context.requestTimestamp });
                    loyaltyStatus = loyaltyResolution.status;
                    loyaltySnapshot = { customer: loyaltyResolution.customer, program: loyaltyResolution.program ? { programId: loyaltyResolution.program.id } : null, pointsEarned: loyaltyResolution.pointsEarned, pointsRedeemed: loyaltyResolution.permittedRedemption, redemptionAmount: loyaltyResolution.redemptionAmount };
                    if (loyaltyResolution.status === 'READY' && loyaltyResolution.customer) {
                        ...
                        const pointsRedeemed = loyaltyResolution.permittedRedemption;
                        if (pointsRedeemed > 0)
                            loyaltyBalances.validateSufficientBalance(balance, pointsRedeemed);
                        const balanceAfterRedemption = balance.availablePoints - pointsRedeemed;
                        const balanceAfterEarn = balanceAfterRedemption + loyaltyResolution.pointsEarned;
                        if (pointsRedeemed > 0 || loyaltyResolution.pointsEarned > 0)
                            loyaltyBalances.updateBalance(transaction, balance, loyaltyResolution.pointsEarned - pointsRedeemed, 0, context.requestTimestamp);
                        if (pointsRedeemed > 0)
                            loyaltyTransactions.createOnce(transaction, { saleId, organizationId: context.organizationId, customerId: loyaltyResolution.customer.customerId, type: 'REDEEM', points: pointsRedeemed, createdBy: context.authenticatedUserId, idempotencyKey: command.idempotencyKey }, balance.availablePoints, balanceAfterRedemption, context.requestTimestamp);
                        if (loyaltyResolution.pointsEarned > 0)
                            loyaltyTransactions.createOnce(transaction, { saleId, organizationId: context.organizationId, customerId: loyaltyResolution.customer.customerId, type: 'EARN', points: loyaltyResolution.pointsEarned, createdBy: context.authenticatedUserId, idempotencyKey: command.idempotencyKey }, balanceAfterRedemption, balanceAfterEarn, context.requestTimestamp);
                        if (pointsRedeemed > 0 || loyaltyResolution.pointsEarned > 0)
                            loyaltyStatus = 'POSTED';
                    }
                }
                catch (error) {
                    if (command.requestedRedemptionPoints)
                        throw error;
