...
export class TrustedSaleInputResolver {
    ...
    async resolveTrustedSaleInput(command, context) {
        ...
        } const tax = await this.tax.resolveEffectiveTaxForSaleLine({ organizationId: context.organizationId, branchId: command.branchId, categoryId: product.categoryId, productId: product.id, variationId, productTaxProfileId: product.taxProfileId, at: context.requestTimestamp }); const optionSnapshots = await Promise.all(selections.map(async (selection) => { const item = await this.options.getById(context.organizationId, selection.optionItemId); await this.assignments.validateOptionGroupAssigned(context.organizationId, command.branchId, product.id, variationId, item.modifierGroupId); const group = await this.groups.getById(context.organizationId, item.modifierGroupId); const option = await this.options.resolveOptionItemSnapshot(context.organizationId, command.branchId, group, item.id, selection.quantity ?? 1); const taxSnapshot = await this.tax.resolveEffectiveTaxForSaleLine({ organizationId: context.organizationId, branchId: command.branchId, categoryId: product.categoryId, productId: product.id, variationId, optionId: option.optionItemId, productTaxProfileId: option.taxProfileId ?? product.taxProfileId, at: context.requestTimestamp }); return { ...option, taxSnapshot }; })); return { quantity: line.quantity, product: await this.products.resolveProductSnapshot(context.organizationId, command.branchId, product.id), category, ...(variation ? { variation } : {}), unitPrice, ...(recipe ? { recipe } : {}), options: optionSnapshots, tax, ...(line.notes ? { notes: line.notes } : {}) }; }));
