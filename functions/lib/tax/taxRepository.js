...
export class TaxRepository {
    ...
    async resolveBranchOverride(organizationId, branchId, at) { return this.assignment(organizationId, 'BRANCH', branchId, at); }
    resolveCategoryTax(organizationId, categoryId, at) { return this.assignment(organizationId, 'CATEGORY', categoryId, at); }
    resolveProductTax(organizationId, productId, at) { return this.assignment(organizationId, 'PRODUCT', productId, at); }
    resolveVariationTax(organizationId, variationId, at) { return this.assignment(organizationId, 'VARIATION', variationId, at); }
    resolveOptionTax(organizationId, optionId, at) { return this.assignment(organizationId, 'OPTION', optionId, at); }
    ...
    validateEffectivePeriod(version, at = new Date()) { if (!version.active || !isEffective(version.effectiveFrom, version.effectiveTo, at))
        throw new HttpsError('failed-precondition', 'Tax version is not effective.'); if (!['TAX_INCLUSIVE', 'TAX_EXCLUSIVE', 'NOT_APPLICABLE'].includes(version.calculationMode) || !/^\d+(?:\.\d+)?$/.test(version.rate) || !Number.isFinite(Number(version.rate)) || Number(version.rate) < 0)
        throw new HttpsError('failed-precondition', 'Tax configuration is invalid.'); }
