...
export class TaxResolver {
    ...
    async resolveEffectiveTaxForSaleLine(input) {
        ...
        if (!profile.active || (profile.branchIds?.length && !profile.branchIds.includes(input.branchId)))
            throw new HttpsError('failed-precondition', 'Tax profile is not applicable to this branch.');
